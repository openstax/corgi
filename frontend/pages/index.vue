                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                  <v-row>
                    <v-radio-group v-model="jobType" row mandatory :default="jobTypes.PDF" @change="resetFormValidation">
                      ...
                      <v-radio label="Docx (git)" :value="jobTypes.GIT_DOCX" class="git-docx-radio-button" />
                    </v-radio-group>
                  </v-row>
                  <v-row>
      ...
      <v-row>
        ...
        >
          ...
          <template #item.version="{ item }">
            ...
            <a v-if="item.job_type_id > 2" :href="collectionURL(item)">{{ ref(item) }}</a>
          </template>
...
export default {
  ...
  beforeCreate () {
    ...
    const validRepoRegEx = new RegExp('^([' + pL + '0-9-_]+/){1,2}[' + pL + '0-9-_]+$')
