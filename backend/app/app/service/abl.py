...
def get_book_metadata(repo_name, slug, version="main"):
    # commit_sha
    refs = requests.get(f"https://api.github.com/repos/openstax/{repo_name}/git/refs/heads/{version}", headers)
    ...
    # uuid    
    collection_xml = get_public_git_file("openstax", repo_name, f"/collections/{slug}.collection.xml", version)
