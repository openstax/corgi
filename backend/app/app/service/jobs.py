...
async def get_or_create_repository(
    ...
    db_repo = cast(
        Optional[Repository],
        db.query(Repository)
        .filter(Repository.id == github_repo.database_id)
        .first(),
    )
...
class JobsService(ServiceBase):
    ...
    async def create(
        ...
        if repo_book_in is not None and not any(
            b["slug"] == repo_book_in for b in repo_books
        ):
        ...
        async def insert_job():
            ...
            if (
                ...
                if not any(ur.user.id == user.id for ur in db_repo.users):
                    # If not, get the information we need and add the
                    # association. Use `get_repository` to get the
                    # viewer_permission
