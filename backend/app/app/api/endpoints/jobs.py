...
def list_jobs(
    ...
    """List jobs from start until now, up to a maximum of one year delta"""
    ...
    if range_start is None:
        ...
        old_jobs = get_old_jobs_json(
            db, yesterday - timedelta(days=364), yesterday, clear_cache
        )
        new_jobs = ",".join(
            [
                Job.from_orm(j).json()
        ...
        return Response(
            content=f"[{joined_jobs}]", media_type="application/json"
        )
    else:
        return [
            Job.from_orm(j)
...
def list_job_page(
    db: Session = Depends(get_db), page: int = 0, limit: int = 50
):
    """List jobs by page"""
    skip = page * limit
    order_by = jobs_service.schema_model.created_at.desc()
    jobs = jobs_service.get_items_order_by(
...
def get_job(*, db: Session = Depends(get_db), id: int):
    """Get a single job"""
...
@router.post(
    "/", response_model=Job, dependencies=[Depends(RequiresRole(Role.USER))]
)
...
def update_job(*, db: Session = Depends(get_db), id: int, job_in: JobUpdate):
    ...
    if int(job.status_id) in [4, 5, 6]:
        ...
        job_in.status_id = str(
            incoming_status if incoming_status in [4, 5, 6] else job.status_id
        )
...
def get_error(*, db: Session = Depends(get_db), id: int):
    job = jobs_service.get(db_session=db, obj_id=id)
    if job is None:
        raise HTTPException(status_code=404, detail="Job not found")
