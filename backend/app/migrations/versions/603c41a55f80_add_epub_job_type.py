...
def downgrade():
    bind = op.get_bind()
    delete_jobs_of_seeded_type = jobs_table.delete().where(jobs_table.c.job_type_id.in_([epub_type_id]))
    delete_seed = job_types_table.delete().where(job_types_table.c.id.in_([epub_type_id]))
