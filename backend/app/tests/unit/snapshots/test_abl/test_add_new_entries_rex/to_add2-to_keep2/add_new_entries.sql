...
WHERE book.uuid IN (__[POSTCOMPILE_uuid_1]) AND approved_book.consumer_id = :consumer_id_1 AND NOT (book.uuid = :uuid_2 AND (commit.sha LIKE :sha_1 || '%') OR book.uuid = :uuid_3 AND (commit.sha LIKE :sha_2 || '%'))
