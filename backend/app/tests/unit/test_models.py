...
def test_repository_summary_zero_commits():
    # GIVEN: A repository containing zero commits
    repo = Repository(id=1, name="test", owner="owner")
    repo.commits = []
    # WHEN: The repository getter tries to get books
    # THEN: It returns an empty list instead of the default (None)
    getter = RepositorySummary.model_validate(repo)
    books = getter.books
    assert books == []


def test_repository_summary_zero_books():
    ...
    repo.commits = [commit]
    commit.books = []
    # WHEN: RepositorySummary is validated from the ORM object
    # THEN: It returns an empty list instead of the default (None)
    books = RepositorySummary.model_validate(repo).books
    assert books == []


def test_repository_summary_one_book():
...
def test_repository_summary_multiple_matches():
