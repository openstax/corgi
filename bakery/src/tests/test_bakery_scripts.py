...
def test_gdocify_book(tmp_path, mocker, requests_mock):
    ...
    # Setup requests mock
    request_path = "/apps/cms/api/v2/pages"
    requests_mock.get(
        f"{request_path}?type=books.Book&fields=slug,cnx_id&cnx_id=bookuuid1",
        json={"items": [{
            "meta": {"slug": "l1-book-slug"},
            "cnx_id": "bookuuid1"
        }]}
    )
    requests_mock.get(
        f"{request_path}?type=books.Book&fields=slug,cnx_id&"
        "cnx_id=random-uuid",
        json={"items": []}
    )

    # Test gen_book_slug_resolver
    book_slug_resolver = gdocify_book.gen_book_slug_resolver(
        requests.Session()
    )
    res = book_slug_resolver("bookuuid1")
    assert res == "l1-book-slug"
    assert requests_mock.call_count == 1
    # Query same slug again to ensure the mocker isn't called
    ...
    with pytest.raises(ValueError):
        res = book_slug_resolver("random-uuid")
    ...
    expected_links_by_id = {
        "l1": "http://openstax.org/books/l1-book-slug/pages/l1-page-slug",
        "l2": "http://openstax.org",
        "l3": "#foobar",
        "l4": "http://www.openstax.org/l/shorturl",
        "l5":
            "http://openstax.org/books/l1-book-slug/pages/l1-page-slug#foobar",
    }
